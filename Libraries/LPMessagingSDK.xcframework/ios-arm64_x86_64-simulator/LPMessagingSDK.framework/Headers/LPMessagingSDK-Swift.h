...
@property (nonatomic) UIBarStyle datePickerHeaderPickerToolbarStyle SWIFT_AVAILABILITY(ios,deprecated=13.0.0,obsoleted=26.0.0,message="Use `datePickerHeaderPickerConfirmationButtonTintColor` instead");
...
/// Used to set the width constraint for Structured Content elements of button type. Should be 0 or above.
...
/// The time-to-live (TTL) duration in minutes for caching base HTTPS service responses.
/// Determines how long the cached responses remain valid before a new network request is made.
/// Default value is 15 minutes.
@property (nonatomic) NSUInteger baseHttpsServicesResponseCacheTtlInMinutes;
...
/// It’s recommended to call this method early in the application’s lifecycle, for example,
/// in your <code>AppDelegate</code>’s <code>application(_:didFinishLaunchingWithOptions:)</code> method.
/// \param brandID The account ID for the LivePerson brand.
///
/// \param monitoringInitParams An object containing parameters required for initializing the monitoring and engagement services,
/// such as the app install ID.
///
/// \param preloadConfiguration A boolean flag that determines
/// whether to fetch the required configurations (ACCDN, LPCDN, LPTag) immediately upon initialization.
/// Defaults to <code>false</code>.
///
/// \param certPinningKeys An array of public key hashes for certificate pinning. If
/// preloadConfiguration is enabled with Certificate pinning, this parameter must be
/// provided for the functionality to work correctly.
///
- (BOOL)initialize:(NSString * _Nullable)brandID monitoringInitParams:(LPMonitoringInitParams * _Nullable)monitoringInitParams preloadConfiguration:(BOOL)preloadConfiguration certPinningKeys:(NSArray<NSString *> * _Nullable)certPinningKeys error:(NSError * _Nullable * _Nullable)error;
/// Initializes the SDK and its components asynchronously on a background thread.
/// It’s recommended to call this method early in the application’s lifecycle,
/// for example, in your <code>AppDelegate</code>’s <code>application(_:didFinishLaunchingWithOptions:)</code> method.
/// Once initialization is complete, the appropriate callback will be invoked on the main thread.
/// \param brandID The account ID for the LivePerson brand. If not provided,
/// the SDK will be initialized without a specific brand context, which may
/// limit functionality until a brand is identified.
///
/// \param monitoringInitParams An optional object containing parameters required
/// for initializing monitoring and engagement services, such as the app install ID.
///
/// \param preloadConfiguration A boolean flag that determines whether to fetch required
/// configurations (like ACCDN, LPCDN, and LPTag) immediately upon initialization.
/// Defaults to <code>false</code>.
///
/// \param certPinningKeys An array of public key hashes for certificate pinning. If
/// preloadConfiguration is enabled with Certificate pinning, this parameter must be
/// provided for the functionality to work correctly.
///
/// \param completion A closure that is executed on the main thread upon successful initialization.
/// It is safe to call <code>showConversation</code> from within this block.
///
/// \param failure A closure that is executed on the main thread if initialization fails,
/// providing an <code>Error</code> object with details about the cause of failure.
///
- (void)initialize:(NSString * _Nullable)brandID monitoringInitParams:(LPMonitoringInitParams * _Nullable)monitoringInitParams preloadConfiguration:(BOOL)preloadConfiguration certPinningKeys:(NSArray<NSString *> * _Nullable)certPinningKeys completion:(void (^ _Nonnull)(void))completion failure:(void (^ _Nonnull)(NSError * _Nonnull))failure;
/// Initialize the SDK and all of its components.
/// It’s recommended to call this method early in the application’s lifecycle, for example,
/// in your <code>AppDelegate</code>’s <code>application(_:didFinishLaunchingWithOptions:)</code> method.
/// \param brandID The account ID for the LivePerson brand.
///
/// \param monitoringInitParams An object containing
/// parameters required for initializing the monitoring and engagement services, such as the app install ID.
///
