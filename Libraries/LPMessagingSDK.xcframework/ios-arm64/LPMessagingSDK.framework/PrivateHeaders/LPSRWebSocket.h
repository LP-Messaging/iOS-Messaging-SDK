 ...
 */
extern NSString *const LPSRWebSocketErrorDomain;

/**
 Key used for HTTP status code if bad response was received from the server.
 */
extern NSString *const LPSRHTTPResponseErrorKey;
 ...
 */
@interface LPSRWebSocket : NSObject <NSStreamDelegate>

/*
 THIS PROPERTY BECAME PUBLIC SO WE CAN USE IT LPMESSAGINGSDK
 */
 ...
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request securityPolicy:(LPSRSecurityPolicy *)securityPolicy;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(nullable NSArray<NSString *> *)protocols securityPolicy:(LPSRSecurityPolicy *)securityPolicy NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURL:(NSURL *)url securityPolicy:(LPSRSecurityPolicy *)securityPolicy;
...
/**
 ...
 By default, a web socket will schedule itself on `+[NSRunLoop LPSR_networkRunLoop]` using `NSDefaultRunLoopMode`.
...
/**
 ...
 An instance of `LPSRWebSocket` is intended for one-time-use only. This method should be called once and only once.
 */
- (void)open;
...
/**
 The `SRWebSocketDelegate` protocol describes the methods that `LPSRWebSocket` objects
...
/**
 ...
 @param webSocket An instance of `LPSRWebSocket` that received a message.
 @param message   Received message. Either a `String` or `NSData`.
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didReceiveMessage:(id)message;
 ...
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didReceiveMessageWithString:(NSString *)string;
 ...
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didReceiveMessageWithData:(NSData *)data;
...
/**
 ...
 @param webSocket An instance of `LPSRWebSocket` that was open.
 */
- (void)webSocketDidOpen:(LPSRWebSocket *)webSocket;

/**
 Called when a given web socket encountered an error.

 @param webSocket An instance of `LPSRWebSocket` that failed with an error.
 @param error     An instance of `NSError`.
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didFailWithError:(NSError *)error;

/**
 Called when a given web socket was closed.
 
 @param webSocket An instance of `LPSRWebSocket` that was closed.
 ...
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(nullable NSString *)reason wasClean:(BOOL)wasClean;

/**
 Called on receive of a ping message from the server.

 @param webSocket An instance of `LPSRWebSocket` that received a ping frame.
 @param data      Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didReceivePingWithData:(nullable NSData *)data;

/**
 Called when a pong data was received in response to ping.

 @param webSocket An instance of `LPSRWebSocket` that received a pong frame.
 @param pongData  Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(LPSRWebSocket *)webSocket didReceivePong:(nullable NSData *)pongData;
...
/**
 ...
 @param webSocket An instance of `LPSRWebSocket` that received a text frame.

 @return `YES` if text frame should be converted to UTF-8 String, otherwise - `NO`. Default: `YES`.
 */
- (BOOL)webSocketShouldConvertTextFrameToString:(LPSRWebSocket *)webSocket NS_SWIFT_NAME(webSocketShouldConvertTextFrameToString(_:));
